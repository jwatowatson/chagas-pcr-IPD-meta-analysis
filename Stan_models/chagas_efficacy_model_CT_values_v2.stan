...
parameters {
  ...
  // treatment effects
  real<lower=-5,upper=5> lambda_trt;  // treatment effect parameterised as a shift in mean CT value
...
model {
  ...
  lambda_trt ~ normal(0.0 , 2.0) T[-5,5];
  sigma_PCR ~ normal(0, 0.5) T[0,];
  alpha_PCR ~ normal(0, 0.5) T[0,];
  sigma_sample ~ normal(1, 1) T[0,];
  sigma_time ~ normal(1,1) T[0,];
  
  // hierachical model of the baseline values
  CT_pop_baseline ~ uniform(CTmin, CTmax);
