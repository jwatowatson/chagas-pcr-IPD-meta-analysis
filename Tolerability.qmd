...
```{r treatment_discont, fig.height=10, fig.width=10}
dis_data = pcr_chagas %>%  
...
names(my_pal)=levels(dis_data$ARM2)
...
# ggsurvplot(s1, data = bendita_dis, risk.table = T,fun = function(x) 1-x,
#            title='Pausing treatment',ylim=c(0,0.4),legend='top',
#            legend.labs=levels(droplevels(bendita_dis$ARM2)),
#            legend.title='', palette = my_pal)
...
# s1 <- survfit(Surv(time_pause, paused) ~ ARM2, data = e1224_dis)
# ggsurvplot(s1, data = e1224_dis, risk.table = T,fun = function(x) 1-x,
#            title='Pausing treatment',ylim=c(0,0.4),legend='top',
#            legend.labs=levels(droplevels(e1224_dis$ARM2)),
#            legend.title='', palette = my_pal)
...
Trying to understand pausing treatment in the 2 week arm

```{r}
ids_2wks_pause = dis_data %>% 
  filter(paused==1,STUDYID=='BENDITA',ARM2=='300mg-2wks') %>%
  select(USUBJID, time_stop, time_pause, BNZ_total_dose)
in_chagas_pause = in_chagas %>% 
  filter(USUBJID %in% ids_2wks_pause$USUBJID,
         INTRT%in%c('PLACEBO','BENZNIDAZOLE')) %>%
  arrange(USUBJID, IN_Day_frm_rand, INTRT)

in_chagas_pause = 
  merge(in_chagas_pause, ids_2wks_pause, by = 'USUBJID',all.x = T)
