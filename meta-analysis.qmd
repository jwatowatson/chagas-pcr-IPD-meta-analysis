...
```{r treatment_discont, fig.height=8, fig.width=8}
bendita_dis = pcr_chagas %>% filter(STUDYID=='BENDITA') %>% 
  distinct(ID, .keep_all = T) %>%
  mutate(
    discont = ifelse(!is.na(time_stop),1,0),
    paused = ifelse(!is.na(time_pause),1,0),
    time_stop = ifelse(is.na(time_stop), 56, time_stop),
    time_stop = ifelse(time_stop>56, 56, time_stop),
    time_pause = ifelse(is.na(time_pause), 56, time_pause),
    time_pause = ifelse(time_pause>56, 56, time_pause),
    ARM2 = case_when(
...
p1=ggsurvplot(s1, data = bendita_dis, risk.table = F,fun = function(x) 1-x,
              title='Stopping treatment',ylim=c(0,0.3),legend='top')

s1 <- survfit(Surv(time_pause, paused) ~ ARM2, data = bendita_dis)
p2=ggsurvplot(s1, data = bendita_dis, risk.table = F,fun = function(x) 1-x,
              title='Pausing treatment',ylim=c(0,0.5),legend='none')
p1$plot
p2$plot
# 
# e1224_dis = pcr_chagas %>% filter(STUDYID=='E1224') %>% 
#   distinct(ID, .keep_all = T) %>%
#   mutate(
#     discont_paused = ifelse(!is.na(time_stop),1,0),
#     time_stop = ifelse(is.na(time_stop), 56, time_stop),
#     time_stop = ifelse(time_stop>56,56, time_stop))
# s1 <- survfit(Surv(time_stop, discont_paused) ~ ARMCD, data = e1224_dis)
# ggsurvplot(s1, data = bendita_dis, risk.table = F,fun = function(x) 1-x)
...
Order individuals by how many days BNZ they got

```{r days_bnz}
pcr_chagas_ss = pcr_chagas %>%
  filter(FOS_total_dose==0, FEX_total_dose==0, 
         STUDYID=='BENDITA',BNZ_total_days<=4*7,
         N_FUP_VISITS>2) %>%
  arrange(desc(BNZ_total_days), Mean_CT_inv_baseline, Day_frm_rand)
pcr_mat = make_matrix_pcr(pcr_dat = pcr_chagas_ss,N_max_PCR = 9)
...
plot_pcr_matrix(pcr_mat = pcr_mat,
                ...
                my_break_legend = my_breaks_legend,
                my_cols = my_cols,
                arm_labels = unique(pcr_chagas_ss$BNZ_total_days),
                h_lines_ind = ind_y[-1]-0.5,
                y_lab_ind = ind_y + diff(c(ind_y, nrow(pcr_mat)+1))/2,
...
pcr_chagas_ss = pcr_chagas %>% 
  filter(!is.na(N_pos_sample), 
         !is.na(N_PCRs_sample), 
         VISIT_numeric>EOT,VISIT_numeric>30,
         STUDYID=='BENDITA',
         !USUBJID %in% c('681'))
        # !USUBJID %in% c('594','681','342')) 
